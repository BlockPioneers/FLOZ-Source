...
// FLOZ: optional setting to unlock wallet for block minting only;
...
// FLOZ: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // FLOZ: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // FLOZ: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // FLOZ: not avatar mode
                        {
...
// FLOZ: create coin stake transaction
...
// FLOZ: check 'spent' consistency between wallet and txindex
// FLOZ: fix wallet spent state according to txindex
...
// FLOZ: disable transaction (only for coinstake)
